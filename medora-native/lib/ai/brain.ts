...
const buildGlobalContext = (query: string): string => {
  ...
  const recentReports = reports
    .slice(0, 3)
    .map((r) => `- ${r.title} (${r.date}) by ${r.doctor}`)
    .join("\n");
  context += `Recent Clinical Reports:\n${recentReports}\n\n`;

  const diet = dietMeals
    .slice(0, 2)
    .map((m) => `- ${m.name}: ${m.clinicalRationale}`)
    .join("\n");
...
const detectIntent = (query: string): AIIntent => {
  ...
  if (["cook", "recipe", "diet", "eat", "meal", "nutrition"].some((k) => q.includes(k)))
    return "DIET";
  if (["pain", "ache", "feel", "hurt", "dizzy", "tired", "symptom"].some((k) => q.includes(k)))
    return "SYMPTOM";
  if (["pill", "dose", "medication", "side effect", "take"].some((k) => q.includes(k)))
    return "MEDICINE";
  if (
    ["why", "how", "what", "understand", "explain", "lab", "report", "test"].some((k) =>
      q.includes(k),
    )
  )
    return "DOUBT";
