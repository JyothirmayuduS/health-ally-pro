    ...
    <Animated.View style={animStyle}>
      ...
      >
        <View
          style={[
            ds.docAvatarWrap,
            { backgroundColor: selected ? "rgba(255,255,255,0.12)" : colors.surface },
          ]}
        >
          <Text
            style={[
        ...
        <View style={ds.docInfo}>
          <Text
            style={[ds.docName, { color: selected ? colors.primaryForeground : colors.foreground }]}
            numberOfLines={1}
          >
            {doctor.name}
          </Text>
          <Text
            style={[ds.docSpec, { color: selected ? "rgba(255,255,255,0.6)" : colors.inkMuted }]}
          >
            {doctor.specialty}
          </Text>
          <View style={ds.docMeta}>
            <Star
              size={10}
              color={selected ? "#F0C97A" : colors.clay}
              fill={selected ? "#F0C97A" : colors.clay}
            />
            <Text
              style={[
                ds.docRating,
                { color: selected ? "rgba(255,255,255,0.7)" : colors.inkMuted },
              ]}
            >
              {doctor.rating}
            </Text>
            <View
              style={[
                ds.dot,
                { backgroundColor: selected ? "rgba(255,255,255,0.3)" : colors.border },
              ]}
            />
            <Text
              style={[
                ds.docRating,
                { color: selected ? "rgba(255,255,255,0.5)" : colors.inkMuted },
              ]}
              numberOfLines={1}
            >
              {doctor.hospital}
        ...
        <View
          style={[
            ds.checkCircle,
            {
              backgroundColor: selected ? "#4CAF7D" : colors.surface,
              borderColor: selected ? "#4CAF7D" : colors.border,
            },
          ]}
        >
          {selected && (
...
function DoctorStatusCard({
  ...
  return (
    <Animated.View
      style={[animStyle, dsc.card, { backgroundColor: colors.surface, borderColor: colors.border }]}
    >
  ...
  return (
    ...
    >
      <Animated.View
        ...
        style={{ alignItems: "center", width: "100%", gap: 32 }}
      >
          ...
          <View
            style={[
              ps.iconNode,
              {
                ...
                borderColor: stage === "Preparing" ? colors.clay : colors.border,
              },
            ]}
          >
            <User
              size={22}
              color={stage === "Preparing" ? colors.clay : colors.inkMuted}
              strokeWidth={1.5}
            />
          </View>
          <View
            style={[ps.dash, { backgroundColor: progress > 33 ? colors.clay : colors.border }]}
          />
          <View
            style={[
              ps.iconNode,
              {
                backgroundColor: colors.surface,
                borderColor: stage === "Encrypting" ? colors.clay : colors.border,
              },
            ]}
          >
            <Clock
              size={22}
              color={stage === "Encrypting" ? colors.clay : colors.inkMuted}
              strokeWidth={1.5}
            />
          </View>
          <View
            style={[ps.dash, { backgroundColor: progress > 66 ? colors.clay : colors.border }]}
          />
          <View
            style={[
              ps.iconNode,
              {
                backgroundColor: colors.surface,
                borderColor: stage === "Transmitting" ? colors.clay : colors.border,
              },
            ]}
          >
            <Stethoscope
              size={22}
              color={stage === "Transmitting" ? colors.clay : colors.inkMuted}
              strokeWidth={1.5}
            />
          </View>
        </View>
...
export default function ReportDetailScreen() {
  ...
  const startSharingSequence = () => {
    ...
    setTimeout(() => {
      setSharingProgress(40);
      setSharingStage("Encrypting");
    }, 800);
    setTimeout(() => {
      setSharingProgress(75);
      setSharingStage("Transmitting");
    }, 1600);
    setTimeout(() => {
  ...
  const simulateResponses = useCallback((sharedIds: string[]) => {
    ...
    const mockResponses: ShareStatus[] = [
      "accepted",
      "accepted",
      "declined",
      "accepted",
  ...
  if (!initial) {
    return (
      <SafeAreaView style={[s.safe, { backgroundColor: colors.background }]}>
        <Text style={{ color: colors.inkMuted, fontFamily: "DMSans_400Regular", padding: 24 }}>
          Report not found.
        </Text>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: colors.background }]} edges={["top"]}>
      ...
      >
        ...
        <Animated.View entering={FadeInDown.duration(500)} style={s.reportHeader}>
          ...
          <Text style={[s.meta, { color: colors.inkMuted }]}>
            {initial.doctor}
            {"  ·  "}
            {new Date(initial.date).toLocaleDateString("en-US", {
              month: "long",
              day: "numeric",
              year: "numeric",
            })}
          ...
          <View style={s.actionRow}>
            <Pressable
              style={[s.ghostBtn, { borderColor: colors.border, backgroundColor: colors.surface }]}
            >
            ...
            <View style={[s.legendRow, { borderTopColor: colors.border }]}>
              {(
                Object.entries(STATUS_CONFIG) as [
                  ShareStatus,
                  (typeof STATUS_CONFIG)[ShareStatus],
                ][]
              ).map(([key, cfg]) => {
              ...
              {labResults.map((row, i) => (
                ...
                >
                  ...
                  <View style={s.labRight}>
                    ...
                    <View
                      style={[s.flagPill, { backgroundColor: row.ok ? "#4CAF7D18" : "#B6785C18" }]}
                    >
                      <Text style={[s.flagText, { color: row.ok ? "#4CAF7D" : "#B6785C" }]}>
                        {row.flag}
                      </Text>
                    </View>
                  </View>
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(220)}>
          <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <View>
                ...
                <Text style={[s.accessSub, { color: colors.inkMuted }]}>
                  {sharedDoctors.length === 0
                    ...
                    : `Shared with ${sharedDoctors.length} doctor${sharedDoctors.length > 1 ? "s" : ""}`}
                </Text>
              </View>
            </View>
            {sharedDoctors.length > 0 && (
                ...
                {sharedDoctors.map((d) => (
                  ...
                  >
                    ...
                    <Pressable
                      ...
                      style={[s.revokeBtn, { borderColor: colors.border }]}
                    >
            ...
            >
              ...
              <Text style={[s.auditItem, { color: colors.inkMuted }]}>
                · Uploaded {new Date(initial.date).toLocaleDateString()}
              </Text>
              <Text style={[s.auditItem, { color: colors.inkMuted }]}>
                · Encrypted with AES-256
              </Text>
              <Text style={[s.auditItem, { color: colors.inkMuted }]}>
                · {sharedDoctors.length} active access grant{sharedDoctors.length !== 1 ? "s" : ""}
              </Text>
            </View>
          </View>
        </Animated.View>
            ...
            >
              ...
              <View style={s.sheetHeader}>
                <View>
                  ...
                  <Text style={[s.sheetSub, { color: colors.inkMuted }]} numberOfLines={1}>
                    {initial.title}
                  </Text>
                </View>
                <Pressable
              ...
              <View style={[s.expiryRow, { borderTopColor: colors.border }]}>
                <View style={s.expiryLeft}>
                  ...
                  <View>
                    <Text style={[s.expiryLabel, { color: colors.foreground }]}>
                      Access expires in
                    </Text>
                    <Text style={[s.expirySub, { color: colors.inkMuted }]}>
                ...
                <View style={s.stepperRow}>
                  <Pressable
                    onPress={() => setExpiry((e) => Math.max(1, e - 1))}
                    style={[
                      s.stepBtn,
                      { backgroundColor: colors.background, borderColor: colors.border },
                    ]}
                  ...
                  <Pressable
                    onPress={() => setExpiry((e) => Math.min(30, e + 1))}
                    style={[
                      s.stepBtn,
                      { backgroundColor: colors.background, borderColor: colors.border },
                    ]}
              ...
              <Pressable
                ...
                style={[
                  s.doneBtn,
                  { backgroundColor: colors.ink, opacity: shared.length === 0 ? 0.35 : 1 },
                ]}
      ...
      {isSharing && (
        <ShareProgressModal progress={sharingProgress} stage={sharingStage} colors={colors} />
      )}
    </SafeAreaView>
  );
}
