            ...
            <View style={[s.whyCard, { backgroundColor: colors.surface }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[s.whyContext, { color: colors.inkMuted }]}>
                  Prescribed by {med.prescribedBy} to actively manage and stabilize your condition
                  over the long term.
                </Text>
              </View>
            </View>
          ...
          {((med.sideEffects?.length ?? 0) > 0 || (med.interactions?.length ?? 0) > 0) && (
            <Animated.View entering={FadeInUp.duration(500).delay(280)} style={{ marginTop: 24 }}>
              ...
              <View
                style={[
                  s.precautionsCard,
                  { backgroundColor: colors.surface, borderColor: "#E5A059" + "40" },
                ]}
              >
                {(med.interactions?.length ?? 0) > 0 && (
                  <View style={s.precautionBlock}>
                    <View style={s.precautionHeader}>
                      ...
                      <Text style={[s.precautionTitle, { color: colors.foreground }]}>
                        Do not combine with
                      </Text>
                    </View>
                    {med.interactions?.map((interaction, i) => (
                      <Text key={`int-${i}`} style={[s.listItem, { color: colors.inkMuted }]}>
                        • {interaction}
                      </Text>
                    ))}
                  </View>
                )}
                {(med.interactions?.length ?? 0) > 0 && (med.sideEffects?.length ?? 0) > 0 && (
                  <View style={[s.pDivider, { backgroundColor: colors.border }]} />
                )}
                {(med.sideEffects?.length ?? 0) > 0 && (
                  <View style={s.precautionBlock}>
                    ...
                    {med.sideEffects?.map((effect, i) => (
                      <Text key={`ef-${i}`} style={[s.listItem, { color: colors.inkMuted }]}>
                        • {effect}
                      </Text>
                    ))}
                  </View>
            ...
            <View style={[s.adherenceCard, { backgroundColor: colors.surface }]}>
              ...
              <View style={[s.progressBarBg, { backgroundColor: colors.border }]}>
                <Animated.View
                  style={[
                    s.progressBarFill,
                    { backgroundColor: adhereColor(adherence), width: `${adherence}%` },
                  ]}
                />
              </View>
            </View>
              ...
              <View
                style={[
                  s.refillCard,
                  {
                    ...
                    borderColor: med.pillsRemaining <= 7 ? "#D35E50" : colors.border,
                    borderWidth: med.pillsRemaining <= 7 ? 1.5 : 0,
                  },
                ]}
              >
                <View style={s.refillHeader}>
                  ...
                  <Text
                    style={[
                      s.refillCount,
                      { color: med.pillsRemaining <= 7 ? "#D35E50" : colors.ink },
                    ]}
                  >
                ...
                <View
                  style={[
                    s.progressBarBg,
                    { backgroundColor: colors.border, marginTop: -4, marginBottom: 16, height: 6 },
                  ]}
                >
                  <Animated.View
                    style={[
                      s.progressBarFill,
                      {
                        backgroundColor: med.pillsRemaining <= 7 ? "#D35E50" : colors.clay,
      ...
      >
        ...
        >
          ...
          <Text
            style={[
              s.actionBtnText,
              { color: isTaken ? colors.inkMuted : colors.primaryForeground },
            ]}
          >
