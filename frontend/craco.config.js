...
webpackConfig.devServer = (devServerConfig) => {
  // Migrate CRA's webpack-dev-server v4 API to v5 (resolutions forces v5)
  ...
  // v5 replaced `https` with `server: { type: 'https' }`
  if (devServerConfig.https !== undefined) {
    if (devServerConfig.https) {
      devServerConfig.server = { type: "https", ...(typeof devServerConfig.https === "object" ? { options: devServerConfig.https } : {}) };
    }
    delete devServerConfig.https;
  }
  // v5 removed these legacy options
