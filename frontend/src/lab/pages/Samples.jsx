...
import { SectionLabel, StatusPill, EmptyState } from "@/lab/components/Pills";
import { Boxes, Snowflake } from "lucide-react";

const STAGES = ["ordered", "collected", "processing", "validation", "validated"];
const STAGE_LABEL = { ordered: "Ordered", collected: "Collected", processing: "Received", validation: "Processed", validated: "Released" };

export default function Samples() {
  const { orders, patients } = useLab();
  const active = useMemo(() => orders.filter((o) => o.status !== "cancelled")
    .sort((a, b) => new Date(b.ordered_at) - new Date(a.ordered_at)), [orders]);
  const stageIndex = (status) => (status === "validated" ? 4 : STAGES.indexOf(status));

  return (
    <div className="space-y-6" data-testid="samples-page">
      <SectionLabel action={<div className="text-xs font-mono uppercase tracking-wider text-stone-500">{active.length} active samples</div>}>
        Sample &amp; chain-of-custody tracking
      </SectionLabel>

      {active.length === 0 ? (
        <div className="bg-white rounded-xl border border-stone-200 p-8">
          <EmptyState icon={Boxes} title="No samples yet" hint="Active samples appear here once orders are placed." />
        </div>
      ) : (
                ...
                <div key={o.id} className="border-b border-stone-100 pb-5 last:border-0 last:pb-0" data-testid={`sample-${o.id}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <div className="text-xs text-stone-500">{p?.name} · {o.test_code} · {p?.mrn}</div>
                    </div>
                    ...
                    {STAGES.map((s, i) => (
                      <div key={s} className="flex items-center gap-2 flex-1">
                        <div className="flex flex-col items-center">
                          <div className={i <= idx ? "h-3 w-3 rounded-full bg-[var(--sage-700)] ring-4 ring-[var(--sage-100)]" : "h-3 w-3 rounded-full bg-stone-200"} />
                  ...
                  <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs text-stone-600">
                    ...
                    <div className="flex items-center gap-1"><Snowflake className="h-3 w-3 text-sky-500" /><span className="text-stone-500">Storage:</span> Rack A-{(o.id.length % 12) + 1}</div>
                  </div>
                </div>
              );
            })}
