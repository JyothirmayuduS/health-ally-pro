...
export default function Processing() {
  const { orders, patients, saveResults, startProcessing, activeStaff } = useLab();
  const [openId, setOpenId] = useState(null);
  const [draft, setDraft] = useState({});

  const queue = useMemo(() => {
    return orders.filter((o) => o.status === "collected" || o.status === "processing");
  }, [orders]);
  ...
  const openOrder = (o) => {
    setOpenId(o.id);
    setDraft({ ...(o.results || {}) });
    if (o.status === "collected") startProcessing(o.id);
  };

  const handleSave = (complete) => {
    saveResults(openId, draft, { complete });
    if (complete) {
      toast.success(`Results submitted for validation`, { description: openId });
      setOpenId(null);
    } else {
      toast(`Draft saved`, { description: openId });
    }
  };

  const hasCritical = currentCat?.parameters?.some((p) => {
    const f = flagValue(p, draft[p.key]);
    return f.level === "critical";
  });

  const allFilled = currentCat?.parameters?.every((p) => {
    const v = draft[p.key];
      ...
      <div className="space-y-6" data-testid="result-entry">
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <div className="text-sm text-stone-600 mt-1">
                {current.test_name} · {currentPatient?.mrn} · {currentPatient?.age}{currentPatient?.sex}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <PriorityPill priority={current.priority} />
              <span className="text-[11px] font-mono text-stone-500">Tech: {activeStaff}</span>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="col-span-3">
                    <Input
                      ...
                      className="bg-white border-stone-200 font-mono"
                      placeholder="—"
                    />
                  </div>
          ...
          <div className="flex gap-2 mt-6">
            ...
            <Button
              className="bg-[var(--sage-700)] hover:bg-[var(--sage-900)] ml-auto"
