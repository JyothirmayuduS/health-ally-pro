...
export default function Dashboard() {
  const { orders, patients, findCatalog, staff, seedSample } = useLab();
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-8" data-testid="lab-dashboard">
      <SectionLabel
        action={
          <div className="flex gap-2">
            {oldestPending && (
              <Button asChild size="sm" variant="outline" className="border-stone-200" data-testid="oldest-pending-btn">
                <Link to="/lab/orders">
      ...
      {orders.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 flex items-center justify-between" data-testid="empty-state">
          <div>
            <div className="font-medium text-amber-900">No orders yet</div>
            <div className="text-sm text-amber-800/80">Seed demo patients to try the full workflow.</div>
          </div>
          {user?.role === "lab_supervisor" && (
            <Button onClick={seedSample} size="sm" className="bg-amber-600 hover:bg-amber-700" data-testid="seed-btn">
              Seed demo patients
            </Button>
          )}
        </div>
                ...
                return (
                  <Link to="/lab/orders" key={o.id} className="flex items-center gap-3 p-3 rounded-lg bg-red-50/50 border border-red-100 hover:bg-red-50 transition">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-[var(--ink)]">{p?.name} <span className="font-mono text-xs text-stone-500">· {p?.mrn}</span></div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-5" data-testid="staff-on-duty">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-display font-semibold text-[var(--ink)]">Team online</h3>
          </div>
          {staff.length === 0 ? (
            <div className="text-sm text-stone-500">No staff yet.</div>
          ) : (
            <div className="space-y-2">
              {staff.slice(0, 6).map((s) => (
                <div key={s.user_id} className="flex items-center gap-3">
                  {s.picture ? (
                    <img src={s.picture} alt="" className="h-8 w-8 rounded-full object-cover" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] font-mono text-stone-500 uppercase tracking-wide">{s.role?.replace("_", " ")}</div>
                  </div>
        ...
        <div className="bg-white rounded-xl border border-stone-200 p-5" data-testid="recent-activity">
          ...
          {recent.length === 0 ? (
            <div className="text-sm text-stone-500">Activity will appear as orders move through the workflow.</div>
          ) : (
