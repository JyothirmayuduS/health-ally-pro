...
import {
  TestTube2, CheckCircle2, XCircle, Printer, UserCheck, Droplets,
} from "lucide-react";

export default function Collection() {
  const { orders, patients, findCatalog, collect, rejectCollect } = useLab();
  ...
  return (
    <div className="space-y-6" data-testid="collection-page">
      <SectionLabel action={<div className="text-xs font-mono uppercase tracking-wider text-stone-500">Phlebotomy · {queue.length} waiting</div>}>
        Sample collection
      </SectionLabel>
      ...
      {queue.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {queue.map((o) => {
            ...
            return (
              <div key={o.id} data-testid={`collect-card-${o.id}`} className="bg-white rounded-xl border border-stone-200 p-5 hover:shadow-sm transition">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="font-mono text-[11px] text-stone-500 uppercase tracking-wider">{o.accession}</div>
                    <div className="font-display text-lg font-semibold text-[var(--ink)] mt-0.5">{p?.name || "—"}</div>
                    <div className="text-xs font-mono text-stone-500">{p?.mrn} · {p?.age}{p?.sex} · ordered {formatRelative(o.ordered_at)}</div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm bg-stone-50 rounded-lg p-3 mb-4 border border-stone-100">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-stone-700">{o.doctor_name}</div>
                  </div>
                  <div>
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="border-stone-200" onClick={() => printLabel(o)} data-testid={`print-${o.id}`}>
                    <Printer className="h-3.5 w-3.5 mr-1.5" /> Label
                  </Button>
                  <Button size="sm" className="bg-[var(--sage-700)] hover:bg-[var(--sage-900)] flex-1" onClick={() => { setCollectFor(o); setNote(""); }} data-testid={`mark-collect-${o.id}`}>
                    <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" /> Mark collected
                  </Button>
                  <Button size="sm" variant="ghost" className="text-red-600 hover:bg-red-50" onClick={() => { setRejectFor(o); setReason(""); }} data-testid={`reject-${o.id}`}>
      ...
      <Dialog open={!!collectFor} onOpenChange={(o) => !o && setCollectFor(null)}>
        <DialogContent data-testid="collect-dialog">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><UserCheck className="h-4 w-4 text-[var(--sage-700)]" /> Verify patient & collect</DialogTitle>
            <DialogDescription>Confirm patient identity before drawing the specimen.</DialogDescription>
          </DialogHeader>
          {collectFor && (() => {
            ...
            return (
              <div className="space-y-4">
                <div className="bg-[var(--sage-50)] border border-[var(--sage-100)] rounded-lg p-4">
                  <div className="text-[10px] font-mono uppercase tracking-wider text-[var(--sage-900)] mb-1">Confirm identity</div>
                  <div className="font-display text-lg font-semibold">{p?.name}</div>
                  <div className="text-sm text-stone-600">{p?.mrn} · {p?.age}{p?.sex} · {p?.phone}</div>
                </div>
                ...
                <div>
                  ...
                  <Input data-testid="collect-note-input" placeholder="Difficult draw, partial sample…" value={note} onChange={(e) => setNote(e.target.value)} />
                </div>
              </div>
            );
          })()}
          <DialogFooter>
            ...
            <Button data-testid="confirm-collect-btn" className="bg-[var(--sage-700)] hover:bg-[var(--sage-900)]"
              onClick={async () => { await collect(collectFor.id, note); setCollectFor(null); }}>
              <Droplets className="h-3.5 w-3.5 mr-1.5" /> Confirm collected
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!rejectFor} onOpenChange={(o) => !o && setRejectFor(null)}>
        <DialogContent data-testid="reject-collect-dialog">
          <DialogHeader>
            ...
            <DialogDescription>Return the sample to the queue with a reason.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <Label className="text-xs">Reason</Label>
            <Textarea data-testid="reject-reason-input" placeholder="Insufficient sample, wrong tube, not fasting…" value={reason} onChange={(e) => setReason(e.target.value)} />
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setRejectFor(null)}>Back</Button>
            <Button data-testid="confirm-reject-btn" className="bg-red-600 hover:bg-red-700" disabled={!reason.trim()}
              onClick={async () => { await rejectCollect(rejectFor.id, reason); setRejectFor(null); }}>
              Reject & re-queue
            </Button>
          </DialogFooter>
        </DialogContent>
