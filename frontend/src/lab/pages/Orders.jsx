...
import { useLab, formatRelative, formatDateTime, getPatient } from "@/lab/store";
import {
...
export default function Orders() {
  const { orders, patients, findCatalog, cancel } = useLab();
  ...
  return (
    <div className="space-y-6" data-testid="orders-inbox">
      <SectionLabel action={
        <Button variant="outline" size="sm" className="border-stone-200" data-testid="orders-count">
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-4 flex flex-wrap gap-3 items-center" data-testid="orders-filters">
        <Input data-testid="orders-search" placeholder="Search patient, MRN, order ID, test code…"
          value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 min-w-[260px] bg-white border-stone-200" />
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-44 border-stone-200" data-testid="filter-status"><SelectValue /></SelectTrigger>
          <SelectContent>
            {STATUS_FILTERS.map((s) => (<SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>))}
          </SelectContent>
        </Select>
        <Select value={priorityFilter} onValueChange={setPriorityFilter}>
          <SelectTrigger className="w-40 border-stone-200" data-testid="filter-priority"><SelectValue /></SelectTrigger>
          <SelectContent>
              ...
              return (
                <tr key={o.id} className="border-b border-stone-100 hover:bg-stone-50/50 cursor-pointer transition" data-testid={`order-row-${o.id}`} onClick={() => setSelectedId(o.id)}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-[11px] font-mono text-stone-500">{o.test_code} · {o.doctor_name}</div>
                  </td>
          ...
          {selected && (
            <>
              <SheetHeader>
                ...
                <div className="text-[11px] font-mono text-stone-500 uppercase tracking-wider">Accession {selected.accession}</div>
              </SheetHeader>

              <div className="mt-6 space-y-5">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-stone-500 mb-1 flex items-center gap-1.5"><UserRound className="h-3 w-3" /> Patient</div>
                  ...
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-stone-500 mb-1 flex items-center gap-1.5"><Stethoscope className="h-3 w-3" /> Ordering doctor</div>
                    <div className="font-medium">{selected.doctor_name}</div>
                    <div className="text-stone-600 text-xs">Source: {selected.source}</div>
                  </div>
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-stone-500 mb-1 flex items-center gap-1.5"><Beaker className="h-3 w-3" /> Test</div>
                  ...
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-stone-500 mb-1 flex items-center gap-1.5"><Tag className="h-3 w-3" /> Sample</div>
                ...
                {selected.notes && (
                  <div className="bg-stone-50 rounded-lg p-3 text-sm border border-stone-200">
                    <div className="text-[10px] font-mono uppercase tracking-wider text-stone-500 mb-1 flex items-center gap-1.5"><FileText className="h-3 w-3" /> Clinical notes</div>
                    <div className="text-[var(--ink)]">{selected.notes}</div>
                  </div>
                )}

                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="border-stone-200" onClick={() => printLabel(selected)} data-testid="drawer-print-label">
                    <Printer className="h-3.5 w-3.5 mr-1.5" /> Print label
                  </Button>
                  {selected.status === "ordered" && (
                    <Button size="sm" className="bg-[var(--sage-700)] hover:bg-[var(--sage-900)]" onClick={() => navigate("/lab/collection")} data-testid="drawer-collect-btn">
                      Go to collection
                    </Button>
                  )}
                ...
                <div>
                  <div className="text-[10px] font-mono uppercase tracking-wider text-stone-500 mb-2 flex items-center gap-1.5"><CalendarClock className="h-3 w-3" /> Timeline</div>
                  <div className="space-y-3 border-l-2 border-stone-200 pl-4 ml-2">
                    {(selected.history || []).map((h, i) => (
                      <div key={i} className="relative">
                        ...
                        <div className="text-xs text-stone-500">{h.actor} · {formatDateTime(h.at)}{h.note ? ` · ${h.note}` : ""}</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <DialogContent data-testid="cancel-dialog">
          ...
          <DialogFooter>
            ...
            <Button data-testid="confirm-cancel-btn" className="bg-red-600 hover:bg-red-700" disabled={!cancelReason.trim()}
              onClick={async () => { await cancel(selected.id, cancelReason); setCancelOpen(false); setCancelReason(""); setSelectedId(null); }}>
              Confirm cancel
            </Button>
          </DialogFooter>
        </DialogContent>
