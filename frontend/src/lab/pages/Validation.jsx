...
export default function Validation() {
            ...
            {currentCat?.parameters.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-3 font-mono text-[15px] font-semibold text-[var(--ink)]">
  ...
  return (
    <div className="space-y-6" data-testid="validation-page">
      ...
      {released.length > 0 && (
        <div className="bg-white rounded-xl border border-stone-200 p-5" data-testid="recently-released">
          <h3 className="font-display font-semibold text-[var(--ink)] mb-3">Recently released</h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {released.map((o) => {
              const p = getPatient(o, patients);
              return (
                <div key={o.id} className="border border-stone-200 rounded-lg p-3 hover:bg-stone-50 cursor-pointer" onClick={() => setPrintOrderId(o.id)} data-testid={`released-${o.id}`}>
                  <div className="text-xs font-mono text-stone-500">{o.id} · {formatDateTime(o.released_at)}</div>
                  <div className="font-medium text-sm mt-0.5">{p?.name} — {o.test_code}</div>
                  <div className="text-xs text-stone-500">Released by {o.validated_by}</div>
