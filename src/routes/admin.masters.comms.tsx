...
import { loadCommsTemplates, saveCommsTemplates, type CommunicationTemplate } from "@/lib/hospital-masters";
  ...
  return (
    <div className="space-y-4" data-testid="admin-comms-templates">
      <p className="text-[13px] text-ink-500">
        Template placeholders: {"{{patient}}"}, {"{{date}}"}, {"{{doctor}}"}, {"{{link}}"} — gateway integration (Twilio / WhatsApp Business) is ops-configured.
      </p>
          ...
          {rows.map((r) => (
            <li key={r.id} className="p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded bg-teal-soft px-2 py-0.5 font-mono text-[10px] uppercase text-teal">{r.channel}</span>
              </div>
