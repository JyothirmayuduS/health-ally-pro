...
const SORT_OPTIONS: { id: SortOption; label: string; hint: string }[] = [
  {
    ...
    hint: "Urgent cases, follow-ups, and open tasks first",
  },
...
const FILTER_SECTIONS = [
  {
    title: "Condition",
    key: "condition",
    options: ["All", "Diabetes", "Hypertension", "Asthma", "Cardiac"],
  },
  { title: "Status", key: "status", options: ["Urgent", "Monitoring", "Stable", "Critical"] },
  {
    title: "Last visit",
    key: "visit",
    options: ["Any time", "Last 7 days", "Last 30 days", "Overdue >30 days"],
  },
...
const PRIMARY_CHIPS = [
  ...
  },
  {
    ...
    icon: Calendar,
    to: "/doctor/patients" as const,
    search: { view: "today" as const, category: "all" as const },
  },
  {
    id: "urgent" as const,
    label: "Urgent",
    icon: AlertTriangle,
    to: "/doctor/patients" as const,
    search: { view: "urgent" as const, category: "all" as const },
  },
  {
    id: "tasks" as const,
    label: "Tasks",
...
const MORE_VIEW_CHIPS = [
  {
    ...
    icon: ClipboardList,
    to: "/doctor/patients" as const,
    search: { view: "panel" as const, category: "follow-up" as const },
  },
  {
    ...
    icon: Calendar,
    to: "/doctor/patients" as const,
    search: { view: "panel" as const, category: "upcoming" as const },
  },
] as const;
...
function DoctorPatientsList() {
  ...
  const patients = useMemo(() => {
    ...
    if (sort === "condition")
      list = [...list].sort((a, b) => a.condition.localeCompare(b.condition));
  ...
  const isChipActive = (chip: { id: string; search?: { view: PanelView; category: string } }) => {
    ...
    if (chip.search.category !== "all")
      return view === chip.search.view && category === chip.search.category;
    ...
    <div className="relative mx-auto w-full max-w-[1400px] space-y-4 pb-4 lg:space-y-5 lg:pb-6">
      ...
      <div className="flex items-center justify-between gap-3 pt-1">
        ...
        <button
          ...
          className="flex items-center gap-1 text-[11px] font-medium text-[#8A8F8C]"
        >
          ...
          return (
            ...
            >
              ...
              >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate text-base font-semibold text-[#1B3B2E]">
                          {patient.name}
                        </p>
                ...
                {patient.alert && (
                  <p className="mt-3 text-sm font-semibold text-[#C45C4A]">{patient.alert}</p>
                )}
                <p className={cn("text-xs text-[#B8735D]", patient.alert ? "mt-1" : "mt-3")}>
                  {patient.timeline}
                </p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  <span
                    className={cn(
                      ...
                      QUEUE_STATUS_STYLE[queueStatus.kind],
                    )}
                  >
                ...
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-semibold text-[#1B3B2E] group-hover:underline">
                          {patient.name}
                        </p>
                    ...
                    <div className="flex flex-wrap gap-1" onClick={(e) => e.stopPropagation()}>
                      {patient.pills.slice(0, 2).map((pill) => {
                        ...
                        return (
                          <span
                            key={pill}
                            className="rounded-md bg-[#F5F2ED] px-2 py-0.5 text-[10px] font-medium text-[#8A8F8C]"
                          >
                            {pill}
                          </span>
                        );
                  ...
                  <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-end gap-1">
                      <Link
                        ...
                        className="grid h-8 w-8 place-items-center rounded-lg border border-[#E8E4DF] text-[#8A8F8C] hover:bg-[#F5F2ED]"
                      >
                        <MessageCircle className="h-3.5 w-3.5" strokeWidth={1.75} />
                      </Link>
                      {tel ? (
      ...
      {patients.length === 0 && (
        <div className="rounded-[20px] border border-dashed border-[#E8E4DF] bg-white/60 px-6 py-12 text-center">
          ...
          <button
            type="button"
            onClick={() =>
              navigate({ to: "/doctor/patients", search: { view: "panel", category: "all" } })
            }
      ...
      <Sheet open={moreViewsOpen} onOpenChange={setMoreViewsOpen}>
        <SheetContent
          side="bottom"
          className="rounded-t-[28px] border-[#E8E4DF] bg-white px-0 pb-8 pt-3 [&>button]:hidden"
        >
      ...
      <Sheet open={filtersOpen} onOpenChange={setFiltersOpen}>
        <SheetContent
          side="bottom"
          className="max-h-[88dvh] overflow-y-auto rounded-t-[28px] border-[#E8E4DF] bg-white px-0 pb-6 pt-3 [&>button]:hidden"
        >
          <div className="mx-auto mb-4 h-1 w-10 rounded-full bg-[#E8E4DF]" />
          <SheetHeader className="px-5 text-left">
            <SheetTitle className="font-serif text-xl font-semibold text-[#1B3B2E]">
          ...
          <div className="mt-4 space-y-5 px-5">
            {FILTER_SECTIONS.map((section) => (
              <div key={section.key}>
                <p className="mb-2.5 text-[11px] font-semibold tracking-[0.08em] text-[#8A8F8C]">
                  {section.title.toUpperCase()}
                </p>
                <div className="flex flex-wrap gap-2">
                  {section.options.map((option) => {
                    const isStatus = section.key === "status";
                    const active = isStatus
                      ? statusFilters.includes(option)
                      : filterValues[section.key] === option;
                    return (
                      <button
                        key={option}
                        type="button"
          ...
          <div className="mt-6 flex gap-3 px-5">
            ...
            <button
              ...
              className="flex-[1.4] rounded-xl bg-[#1B3B2E] py-3 text-sm font-semibold text-white"
            >
              Apply
            </button>
          </div>
