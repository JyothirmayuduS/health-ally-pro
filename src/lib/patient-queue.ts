...
/** Stable demo roster — position 1–6 maps left-to-right on the strip */
...
/** Patient currently in consultation (the one directly before you). */
...
/** Patients already seen today before the current consultation. */
export function patientsSeenBeforeYou(position: number): number {
  return Math.max(0, position - 2);
}

/**
 * Patients blocking your turn right now (only the one in the room).
 * Not the same as queue index − 1 — completed patients don't count.
 */
...
export function getQueueBoardSummary(position: number, total: number): string {
  const blocking = patientsAhead(position);
  const behind = patientsBehind(position, total);
  const seen = patientsSeenBeforeYou(position);

  if (blocking === 0) return `You're #${position} · ${behind} behind you`;
...
export function getLiveQueueContext(
  ...
  return { appointment: inQueue, doctor };
}
