...
export default function AdminDashboard() {
  ...
  const announcements = useMemo(
    () => getAnnouncements().filter((a) => a.status === "active" && a.priority !== "normal"),
    [],
  );
  ...
  const systemAlerts = useMemo(() => {
    ...
    if (unpaidInvoices > 0)
      alerts.push({
        severity: "warning",
        message: `${unpaidInvoices} unpaid invoices outstanding`,
        link: "/admin/revenue",
      });
    const urgentAnns = getAnnouncements().filter(
      (a) => a.status === "active" && (a.priority === "urgent" || a.priority === "emergency"),
    );
    if (urgentAnns.length)
      alerts.push({
        severity: urgentAnns.some((a) => a.priority === "emergency") ? "critical" : "warning",
        message: `${urgentAnns.length} active urgent announcement(s)`,
    ...
    <div className="space-y-6" data-testid="admin-dashboard">
        ...
        <div className="space-y-1.5">
          {systemAlerts.map((a, i) => (
            <Link
              key={i}
              to={a.link}
              className={`flex items-center justify-between gap-3 rounded-lg border px-4 py-2.5 text-[13px] hover:opacity-90 transition ${a.severity === "critical" ? "border-red-300 bg-red-50 text-red-800" : "border-status-waitBorder bg-status-waitBg text-status-waitText"}`}
            >
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          <div key={k.label} className="surface px-5 py-4">
            ...
            <div className={`mt-2 text-3xl font-heading font-semibold tabular-nums ${k.color}`}>
              {k.value}
            </div>
          </div>
        ))}
        ...
        {[
          {
            ...
            sub: "consultations active",
            icon: Activity,
            color: "text-teal",
          },
          {
            label: "Invoices paid today",
            value: paidToday,
            sub: "of total ledger",
            icon: Banknote,
            color: "text-money",
          ...
          },
          {
            ...
            sub: "on service roster",
            icon: ClipboardList,
            color: "text-plum",
          },
        ].map((t) => (
          <div key={t.label} className="surface px-4 py-3 flex gap-3 items-start">
            <div className="mt-0.5 rounded bg-stone-100 p-1.5">
        ...
        <div className="lg:col-span-3 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-px bg-stone-100">
              {quickLinks.map((l) => (
                <Link
                  key={l.to}
                  to={l.to}
                  className="group flex items-center gap-3 bg-white px-4 py-3.5 hover:bg-plum-soft/40 transition-colors"
                >
          ...
          <div className="surface overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-ink-100">
              ...
              <Link to="/admin/doctors" className="text-[12px] text-plum hover:underline">
                View all →
              </Link>
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-ink-100 max-h-[480px]">
              {activityFeed.map((entry) => (
                <div key={entry.id} className="px-4 py-2.5 flex gap-3 items-start">
                  <div
                    className={`mt-0.5 shrink-0 rounded-sm px-1.5 py-0.5 text-[9px] font-bold uppercase border ${MODULE_COLOR[entry.module]}`}
                  >
                    {entry.module}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[10px] text-ink-400 mt-0.5">
                      {entry.staff} · {timeAgo(entry.timestamp)}
                    </div>
                  </div>
                </div>
              ))}
      ...
      {announcements.length > 0 && (
        <div className="space-y-2">
          <div className="text-[11px] font-semibold uppercase tracking-wider text-ink-400 px-1">
            Active announcements
          </div>
          {announcements.map((ann) => (
            <div
              key={ann.id}
              className={`rounded-lg border px-4 py-3 text-[13px] ${ann.priority === "emergency" ? "bg-red-50 border-red-200 text-red-900" : "bg-status-waitBg border-status-waitBorder text-status-waitText"}`}
            >
              <div className="font-semibold flex items-center gap-2">
                {ann.priority === "emergency" && (
                  <span className="rounded bg-red-100 text-red-700 text-[10px] font-bold uppercase px-1.5 py-0.5">
                    Emergency
                  </span>
                )}
                {ann.priority === "urgent" && (
                  <span className="rounded bg-status-waitBg text-status-waitText text-[10px] font-bold uppercase px-1.5 py-0.5 border border-status-waitBorder">
                    Urgent
                  </span>
                )}
                {ann.title}
