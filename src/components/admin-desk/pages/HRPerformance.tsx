...
const HIKE_CONFIG = {
  excellent: {
    label: "Excellent — Hike Recommended",
    pct: "15–20%",
...
export default function AdminHRPerformance() {
  ...
  const excellentCount = PERFORMANCE_DATA.filter(
    (p) => p.hikeRecommendation === "excellent",
  ).length;
  ...
  const exportCSV = () => {
    const header =
      "Name,Role,Department,Category,Leave Days,Attendance%,Review Score,Hike Recommendation";
    const rows = filteredPerf.map(
      (p) =>
    ...
    a.href = url;
    a.download = "hr_performance_report.csv";
    a.click();
    ...
    <div className="space-y-6" data-testid="admin-hr-performance">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="surface px-5 py-4 border-l-4 border-l-amber-400 shadow-soft">
          ...
          <p className="text-3xl font-heading font-bold text-amber-600 mt-1.5 tabular-nums">
            {totalPending}
          </p>
        ...
        <div className="surface px-5 py-4 border-l-4 border-l-teal shadow-soft">
          ...
          <p className="text-3xl font-heading font-bold text-teal mt-1.5 tabular-nums">
            {avgAttendance}%
          </p>
        ...
        <div className="surface px-5 py-4 border-l-4 border-l-plum shadow-soft">
          ...
          <p className="text-3xl font-heading font-bold text-plum mt-1.5 tabular-nums">
            {excellentCount}
          </p>
            ...
            {leaves
              ...
              .map((req) => {
                ...
                return (
                  <div
                    key={req.id}
                    className="px-5 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-bone/10 transition"
                  >
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="font-semibold text-ink-900 text-[13.5px]">
                            {req.staffName}
                          </span>
                          ...
                          <span
                            className={`px-2 py-0.5 rounded border text-[9.5px] font-bold uppercase ${leaveTypeColors[req.leaveType]}`}
                          >
                            {req.leaveType}
                          </span>
                          <span
      ...
      {activeTab === "performance" && (
        <div className="space-y-5">
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            {(Object.entries(HIKE_CONFIG) as [string, (typeof HIKE_CONFIG)["excellent"]][]).map(
              ([key, cfg]) => {
                ...
                return (
                  <div
                    key={key}
                    className={`surface px-4 py-3 flex items-center gap-3 border shadow-soft`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[15px] font-heading font-bold text-ink-900">
                  ...
                  {sortedByLeaves.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded font-mono font-bold text-[13px] ${
                              p.totalLeaveDays >= 10
                                ? "text-red-600"
                                : p.totalLeaveDays >= 5
                                  ? "text-amber-600"
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10.5px] font-semibold ${hikeCfg.cls}`}
                          >
                            <span className={`h-1.5 w-1.5 rounded-full ${hikeCfg.dot}`} />
                            {p.hikeRecommendation === "below-average"
                              ? "No Hike"
                              : `${hikeCfg.pct}`}
                          </span>
                        </td>
            ...
            <div>
              ...
              <p className="text-[12.5px] text-ink-600 mt-1 leading-relaxed">
                {filteredPerf
                  ...
                  .map((p) => p.staffName)
                  .join(", ") || "None in this filter"}{" "}
                have demonstrated outstanding attendance, patient handling, and low unplanned
                leaves. Consider a <strong>15–20% incremental hike</strong> for these staff.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="flex justify-between items-start gap-4">
                <div>
                  <h4 className="font-heading font-bold text-xl text-ink-950">
                ...
                <div
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[11px] font-semibold ${HIKE_CONFIG[selectedStaff.hikeRecommendation].cls}`}
                >
                  <span
                    className={`h-1.5 w-1.5 rounded-full ${HIKE_CONFIG[selectedStaff.hikeRecommendation].dot}`}
                  />
              ...
              <div className="grid grid-cols-3 gap-4 border-t border-b border-stone-100 py-4 font-mono text-center">
                <div>
                  <span className="text-[9.5px] text-ink-400 uppercase font-sans tracking-wider block">
                    Attendance
                  </span>
                  <span className="text-lg font-bold text-ink-900 block mt-1">
                    {selectedStaff.attendancePct}%
                  </span>
                </div>
                <div>
                  <span className="text-[9.5px] text-ink-400 uppercase font-sans tracking-wider block">
                ...
                <div>
                  ...
                  <span className="text-lg font-bold text-ink-900 block mt-1">
                    {selectedStaff.totalLeaveDays} days
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                <h5 className="text-[11px] uppercase font-bold tracking-wider text-ink-400">
                  Leave Breakdown
                </h5>
                <div className="grid grid-cols-3 gap-2 text-center text-[12px] font-mono">
                  <div className="p-3 bg-red-50/50 border border-red-100 rounded-lg">
                    <span className="text-red-700 block font-bold text-[15px]">
                      {selectedStaff.sickLeaves}
                    </span>
                    <span className="text-[10px] text-red-500 uppercase font-sans">Sick</span>
                  </div>
                  <div className="p-3 bg-blue-50/50 border border-blue-100 rounded-lg">
                    <span className="text-blue-700 block font-bold text-[15px]">
                      {selectedStaff.casualLeaves}
                    </span>
                  ...
                  <div className="p-3 bg-amber-50/50 border border-amber-100 rounded-lg">
                    <span className="text-amber-700 block font-bold text-[15px]">
                      {selectedStaff.emergencyLeaves}
                    </span>
                    <span className="text-[10px] text-amber-500 uppercase font-sans">
                      Emergency
                    </span>
                  </div>
                </div>
                  ...
                  {selectedStaff.category === "doctor" && (
                    <div className="flex justify-between items-center pb-2 border-b border-stone-200/60">
                      ...
                      <strong className="font-mono text-ink-900">
                        {selectedStaff.patientsHandled} patients
                      </strong>
                    </div>
                  )}
