...
export default function ShiftReportModal({ open, onOpenChange }: ShiftReportModalProps) {
  ...
  const metrics = useMemo(() => {
    ...
    const pending = shiftOrders.filter(
    ...
    const onTime = completedWithTat.filter((o) => {
      ...
      const elapsed =
        (new Date(o.validated_at).getTime() - new Date(o.ordered_at).getTime()) / 3_600_000;
    ...
    const rejectedReception = orders.filter((o) =>
      o.history.some(
        (h) => h.action.includes("Sample Accepted -") && !h.action.includes("Adequate"),
      ),
    ).length;
    const storedCount = orders.filter((o) => o.sampleStorage !== undefined).length;

    const shiftRejectionsList: Array<{
      orderId: string;
      patientName: string;
      test: string;
      reason: string;
    ...
    return {
      ...
      quality: {
        ...
        qcFail,
        criticalAlertsCount,
        deltaFailuresCount: 0,
        criticals: shiftCriticalsList,
      },
      integrity: {
        rejectedCollection,
        rejectedReception,
        storedCount,
        rejections: shiftRejectionsList,
      },
      reagents: {
        lowOrExpiredCount,
        blockedTestsCount: qcLocks.length,
        reagentIssues: shiftReagentIssuesList,
      },
    };
  ...
  const handleSignOff = () => {
    ...
    toast.success(
      isSupervisor ? "Shift report signed off successfully" : "Shift handover saved as draft",
    );
    onOpenChange(false);
  };
  ...
  const handlePrint = () => {
    const criticalRows =
      metrics.quality.criticals.length > 0
        ? metrics.quality.criticals
            .map(
              (c) =>
                `<div class="row"><span>Patient: ${c.patientName} (${c.parameters})</span><span>Notified: ${c.notifiedPerson} [${c.status}]</span></div>`,
            )
            .join("")
    ...
    const rejectionRows =
      metrics.integrity.rejections.length > 0
        ? metrics.integrity.rejections
            .map(
              (r) =>
                `<div class="row"><span>ID: ${r.orderId} - Patient: ${r.patientName} (${r.test})</span><span>Reason: ${r.reason}</span></div>`,
            )
            .join("")
    ...
    const reagentRows =
      metrics.reagents.reagentIssues.length > 0
        ? metrics.reagents.reagentIssues
            .map(
              (rg) =>
                `<div class="row"><span>Reagent: ${rg.name} (Lot: ${rg.lot})</span><span>Issue: ${rg.issue} (${rg.remaining} tests left)</span></div>`,
            )
            .join("")
        ...
        <div className="space-y-6 my-2 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                disabled
                className="mt-1 bg-stone-50 border-ink-200"
              />
            </div>
          </div>
          ...
          <div className="border border-stone-200 rounded-lg p-4 bg-stone-50/50">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="font-mono text-lg font-bold text-sage">
                  {metrics.throughput.tatComplianceRate}%
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="border border-stone-200 rounded-lg p-4">
              ...
              <ul className="space-y-1.5 text-xs text-ink-600">
                <li className="flex justify-between">
                  <span>QC Runs Passed / Warn:</span>
                  <span className="font-semibold text-ink-900">
                    {metrics.quality.qcPass} / {metrics.quality.qcWarning}
                  </span>
                </li>
                <li className="flex justify-between">
                  <span>QC Failures / Locks:</span>
                  <span className="font-semibold text-red-600">
                    {metrics.quality.qcFail} / {metrics.reagents.blockedTestsCount}
                  </span>
                </li>
                <li className="flex justify-between">
                  <span>Critical Alerts Released:</span>
                  <span className="font-semibold text-ink-900">
                    {metrics.quality.criticalAlertsCount}
                  </span>
                </li>
              </ul>
            </div>
            <div className="border border-stone-200 rounded-lg p-4">
              ...
              <ul className="space-y-1.5 text-xs text-ink-600">
                <li className="flex justify-between">
                  <span>Rejected at Collection:</span>
                  <span className="font-semibold text-ink-900">
                    {metrics.integrity.rejectedCollection}
                  </span>
                </li>
                <li className="flex justify-between">
                  <span>Rejected at Reception:</span>
                  <span className="font-semibold text-ink-900">
                    {metrics.integrity.rejectedReception}
                  </span>
                </li>
                <li className="flex justify-between">
                  <span>Specimens in Bio-Storage:</span>
                  <span className="font-semibold text-ink-900">
                    {metrics.integrity.storedCount}
                  </span>
                </li>
              </ul>
            </div>
          ...
          <div className="space-y-4 border border-stone-200 rounded-lg p-4 bg-stone-50/30">
            <div>
              <h4 className="font-semibold text-xs text-ink-700 uppercase mb-1">
                Critical Values logged ({metrics.quality.criticals.length})
              </h4>
              {metrics.quality.criticals.length === 0 ? (
                ...
                <div className="max-h-20 overflow-y-auto space-y-1 divide-y divide-stone-100">
                  {metrics.quality.criticals.map((c, idx) => (
                    <div key={idx} className="text-xs py-1 text-ink-600">
                      <strong>{c.patientName}</strong>: {c.parameters} ·{" "}
                      <span className="text-[10px] text-ink-400">
                        Notified {c.notifiedPerson} ({c.status})
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div>
              <h4 className="font-semibold text-xs text-ink-700 uppercase mb-1">
                Rejected Sample Reasons ({metrics.integrity.rejections.length})
              </h4>
              {metrics.integrity.rejections.length === 0 ? (
                ...
                <div className="max-h-20 overflow-y-auto space-y-1 divide-y divide-stone-100">
                  {metrics.integrity.rejections.map((r, idx) => (
                    <div key={idx} className="text-xs py-1 text-ink-600">
                      <strong>Order {r.orderId}</strong> ({r.patientName}):{" "}
            ...
            <div>
              <h4 className="font-semibold text-xs text-ink-700 uppercase mb-1">
                Reagent Warnings / Issues ({metrics.reagents.reagentIssues.length})
              </h4>
              {metrics.reagents.reagentIssues.length === 0 ? (
                ...
                <div className="max-h-20 overflow-y-auto space-y-1 divide-y divide-stone-100">
                  {metrics.reagents.reagentIssues.map((rg, idx) => (
                    <div key={idx} className="text-xs py-1 text-ink-600">
                      <strong>{rg.name}</strong> (Lot {rg.lot}):{" "}
                      <span className="text-amber-700">{rg.issue}</span> · {rg.remaining} remaining
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-2 gap-4 border-t pt-4">
            <div>
              ...
              <Input
                value={isSupervisor ? "J. Mensah" : name}
                disabled
                className="mt-1 bg-stone-50 border-ink-200"
              />
            </div>
            <div>
              ...
              <Select value={supervisorCoSign} onValueChange={setSupervisorCoSign}>
                <SelectTrigger className="w-full bg-white border-ink-200 mt-1">
                  <SelectValue
                    placeholder={isSupervisor ? "Select your signature" : "Select Supervisor"}
                  />
                </SelectTrigger>
                <SelectContent>
                  {staff
        ...
        <DialogFooter className="border-t pt-4 flex gap-2">
          ...
          <div className="flex gap-2 ml-auto">
            ...
            <Button onClick={handleSignOff} className="btn-primary">
              ...
              {isSupervisor ? "Sign & Close Shift" : "Save Handover"}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
