      ...
      <div className="surface p-4 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[260px]">
          ...
          <Input
            ...
            className="pl-9 bg-white border-ink-200"
          />
        </div>
        ...
        {filtered.map((t) => (
          <div key={t.code} className="surface p-5" data-testid={`catalog-${t.code}`}>
            ...
            <div className="grid grid-cols-3 gap-3 text-xs bg-stone-50 rounded-lg p-3 mb-3 border border-stone-100">
              <div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 flex items-center gap-1">
                  <Clock className="h-3 w-3" /> TAT
                </div>
                <div className="font-medium">{t.tat_hours}h</div>
              </div>
              <div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 flex items-center gap-1">
                  <Droplet className="h-3 w-3" /> Tube
                </div>
                <div className="font-medium truncate">{t.tube}</div>
              </div>
              <div>
                <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 flex items-center gap-1">
                  <Beaker className="h-3 w-3" /> Prep
                </div>
            ...
            <div className="space-y-1">
              {t.parameters.map((p) => (
                <div
                  key={p.key}
                  className="flex justify-between text-xs py-1 border-b border-stone-100 last:border-0"
                >
