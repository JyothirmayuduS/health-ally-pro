...
export default function Reports() {
  ...
  const byTest = useMemo(
    () => Object.entries(analytics?.by_test || {}).map(([code, count]) => ({ code, count })),
    [analytics],
  );
  const bySection = useMemo(
    () =>
      Object.entries(analytics?.by_section || {}).map(([section, count]) => ({ section, count })),
    [analytics],
  );
  ...
  const byDay = useMemo(
    () =>
      Object.entries(analytics?.by_day || {})
        .sort()
  ...
  return (
    <div className="space-y-6" data-testid="reports-page">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          label="Released"
          value={analytics?.totals?.validated ?? "—"}
          hint="Reports out"
          accent="emerald"
        />
        <KpiCard
        ...
        <KpiCard
          label="Avg TAT"
          value={analytics?.avg_tat_hours != null ? `${analytics.avg_tat_hours}h` : "—"}
          hint="Order → release"
          accent="sky"
        />
      </div>
              ...
              >
                <div>
                  <div className="font-mono text-xs text-ink-400">
                    {c.order_id} · {c.param} = <b className="text-red-700">{c.value}</b>
                  </div>
                </div>
