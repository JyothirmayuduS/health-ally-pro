...
export default function Validation() {
  ...
  const hasCritical = currentCat?.parameters?.some(
    (p) => flagValue(p, current?.results?.[p.key]).level === "critical",
  );
    ...
    return (
      <div className="space-y-6" data-testid="validation-detail">
        ...
        <div className="surface p-6">
          <div className="flex items-start justify-between mb-6 pb-6 border-b border-ink-200">
            <div>
              ...
              <div className="text-sm text-ink-600 mt-1">
                {current.test_name} · ordered by {current.doctor_name} · completed{" "}
                {formatRelative(current.completed_at)}
              </div>
            </div>
          ...
          {!isSupervisor && (
            <div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 text-sm">
              ...
              <span>
                Only <b>Lab Supervisors</b> can validate or reject results.
              </span>
            </div>
          )}
          ...
          {hasCritical && (
            <div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-red-50 border border-red-200 text-red-800 text-sm">
              ...
              <span>
                <b>Critical value</b> present — Dual sign-off required. Enforce clinician phone/EMR
                alert below to unlock release.
              </span>
            </div>
          )}
          ...
          <div className="space-y-1">
            {currentCat?.parameters.map((p) => {
              ...
              const refStr = p.ref_text
                ? p.ref_text
                : `${p.ref_low ?? "—"}–${p.ref_high ?? "—"} ${p.unit}`;
              return (
                <div
                  key={p.key}
                  className={cn(
          ...
          {hasCritical && (
            <div className="mt-6 p-4 rounded-lg border border-red-200 bg-red-50/30 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div>
                  <Label className="text-[10px] uppercase font-bold text-red-900">
                    Notified Person *
                  </Label>
                  <input
          ...
          <div className="mt-6 space-y-3">
            <Label className="text-xs">
              Supervisor comment {rejectMode ? "(required for rejection)" : "(optional)"}
            </Label>
            <Textarea
              ...
              placeholder={
                rejectMode
                  ? "Reason to send back to technician…"
                  : "Internal note (e.g. delta check OK)"
              }
            />
          </div>
              ...
              {rejectMode ? (
                <>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      rejectValid(current.id, comment);
                      setOpenId(null);
                    }}
                ...
                <>
                  ...
                  <Button
                    ...
                    disabled={!isSupervisor || (hasCritical && !notifiedPerson.trim())}
                    onClick={() => {
        ...
        {printOrder && (
          <LabReport
            order={printOrder}
            patient={getPatient(printOrder, patients) ?? null}
      ...
      {queue.length === 0 ? (
        <div className="surface p-8">
          <EmptyState
            ...
            hint="All completed results have been released."
          />
        </div>
      ) : (
          ...
          <table className="w-full text-sm" data-testid="validation-table">
            ...
            <tbody>
              {queue.map((o) => {
                ...
                const flags = cat?.parameters
                  .map((pp) => flagValue(pp, o.results?.[pp.key]))
                  .filter((f) => f.level !== "normal" && f.level !== "empty");
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      "border-b border-stone-100 cursor-pointer transition",
                      hasCrit ? "hover:bg-red-50/40" : "hover:bg-stone-50",
                    )}
                  >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {released.map((o) => {
              ...
              return (
                <div
                  key={o.id}
                  className="border border-ink-200 rounded-lg p-3 hover:bg-stone-50 cursor-pointer"
                  onClick={() => setPrintOrderId(o.id)}
                  data-testid={`released-${o.id}`}
                >
                  <div className="text-xs font-mono text-ink-400">
                    {o.id} · {formatDateTime(o.released_at)}
                  </div>
                  <div className="font-medium text-sm mt-0.5">
                    {p?.name} — {o.test_code}
                  </div>
                  <div className="text-xs text-ink-400">Released by {o.validated_by}</div>
                  <div className="text-xs text-[var(--sage-700)] mt-1 hover:underline">
                    Print report →
                  </div>
                </div>
              );
