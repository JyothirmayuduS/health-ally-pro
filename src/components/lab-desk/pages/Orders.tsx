...
import { PriorityPill, StatusPill, SectionLabel, EmptyState } from "@/components/lab-desk/Pills";
    ...
    <div className="space-y-6" data-testid="orders-inbox">
      ...
      <div className="surface p-4 flex flex-wrap gap-3 items-center" data-testid="orders-filters">
        <Input
          ...
          className="flex-1 min-w-[260px] bg-white border-ink-200"
        />
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-44 border-ink-200" data-testid="filter-status">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={priorityFilter} onValueChange={setPriorityFilter}>
          <SelectTrigger className="w-40 border-ink-200" data-testid="filter-priority">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
              ...
              <tr>
                <td colSpan={7}>
                  <EmptyState
                    ...
                    hint="Adjust filters or check back later."
                  />
                </td>
              </tr>
            )}
            ...
            <>
              ...
              <div className="mt-6 space-y-5">
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 mb-1 flex items-center gap-1.5">
                      <UserRound className="h-3 w-3" /> Patient
                    </div>
                    <div className="font-medium">{selectedPatient?.name}</div>
                    <div className="text-ink-600 text-xs">
                      {selectedPatient?.mrn} · {selectedPatient?.age}
                      {selectedPatient?.sex} · {selectedPatient?.phone}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 mb-1 flex items-center gap-1.5">
                  ...
                  <div>
                    ...
                    <div className="text-ink-600 text-xs font-mono">
                      {selected.test_code} · TAT {selectedCat?.tat_hours}h
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 mb-1 flex items-center gap-1.5">
                    ...
                    <div className="text-ink-600 text-xs">
                      Tube: {selectedCat?.tube}
                      {selected.fasting ? " · Fasting required" : ""}
                    </div>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="rounded-lg border border-sage/30 bg-sage-soft/20 p-3 text-sm">
                      ...
                      <div className="mt-1 text-xs capitalize text-ink-500">
                        Status: {inv.status}
                      </div>
                      {inv.status === "unpaid" && (
                        <div className="mt-3 flex gap-2">
                          <Button
                            ...
                            onClick={() => collectLabPayment(inv.id, "cash")}
                          >
                            Collect cash
                          </Button>
                          <Button
                            ...
                            onClick={() => flagInvoiceForReception(inv.id)}
                          >
                            Bill at reception
                          </Button>
                        </div>
                ...
                <div className="flex gap-2">
                  ...
                  {selected.status === "ordered" && (
                    <Button
                      size="sm"
                      className="btn-primary"
                      onClick={() => navigate({ to: "/lab/collection" })}
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <DialogContent data-testid="cancel-dialog">
          <DialogHeader>
            ...
            <DialogDescription>
              Provide a reason — this will mark the order as cancelled.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <Textarea
              ...
              placeholder="Duplicate, patient refused, wrong test…"
            />
          </div>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                cancel(selected!.id, cancelReason);
                setCancelOpen(false);
                setCancelReason("");
                setSelectedId(null);
              }}
