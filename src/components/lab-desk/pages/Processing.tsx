...
export default function Processing() {
  ...
  const queue = useMemo(
    () => orders.filter((o) => o.status === "collected" || o.status === "processing"),
    [orders],
  );
  ...
  const openOrder = (o: import("@/lib/lab-desk/mockData").LabOrder) => {
    setOpenId(o.id);
    setDraft({ ...((o.results as Record<string, string>) || {}) });
  };
  ...
  const hasCritical = currentCat?.parameters?.some(
    (p) => flagValue(p, draft[p.key]).level === "critical",
  );
  const allFilled = currentCat?.parameters?.every(
    (p) => draft[p.key] !== undefined && draft[p.key] !== "" && draft[p.key] !== null,
  );
        ...
        <div className="surface p-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              <div className="text-[10px] font-mono uppercase tracking-wider text-ink-400 mb-1">
                {current.accession} · Processing
              </div>
              <h2 className="font-display text-2xl font-semibold text-ink-900">
                {currentPatient?.name} — {current.test_code}
              </h2>
              <div className="text-sm text-ink-600 mt-1">
                {current.test_name} · {currentPatient?.mrn} · {currentPatient?.age}
                {currentPatient?.sex}
              </div>
            </div>
          ...
          {current.priority === "stat" && hasCritical && (
            <div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-red-100 border border-red-300 text-red-900 text-sm animate-pulse-border">
              ...
              <span>
                <strong>STAT ESCALATION ACTIVE:</strong> Critical result detected. Order priority
                has been escalated to STAT for immediate validation.
              </span>
            </div>
          )}
          ...
          {reagentStatus.blocked && (
            <div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-red-50 border border-red-200 text-red-800 text-sm">
              ...
              <span>
                <strong>Hard Block:</strong> Expired or out-of-stock reagent lot detected. Reagent:{" "}
                {reagentStatus.list.map((r) => r.name).join(", ")}. Cannot save results.
              </span>
            </div>
          )}
          ...
          {!reagentStatus.blocked && reagentStatus.warning && (
            <div className="mb-4 flex items-center gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 text-sm">
              ...
              <span>
                <strong>Warning:</strong> Low reagent stock remaining (&lt; 20%) for{" "}
                {reagentStatus.list.map((r) => r.name).join(", ")}. Proceed with caution.
              </span>
            </div>
          )}
            ...
            if (hasOverride) {
              return (
                <div className="mb-4 flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 text-sm">
                  ...
                  <div>
                    <span className="font-bold">Supervisor override in effect.</span> Specimen
                    condition: <span className="font-mono font-bold">{cond}</span>. Processing is
                    allowed.
                    <div className="text-xs text-amber-700 mt-0.5">
                      Overridden by {current.sampleConditionOverride!.overriddenBy} · Reason:{" "}
                      {current.sampleConditionOverride!.reason}
                    </div>
                  </div>
                </div>
              );
            ...
            return (
              <div className="mb-4 space-y-3 p-4 rounded-xl border-2 border-red-300 bg-red-50 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <div className="font-bold text-red-900 text-base">
                      🚫 Hard Block — Non-Adequate Specimen
                    </div>
                    <div className="text-red-800 mt-0.5">
                      Specimen condition recorded as{" "}
                      <span className="font-mono font-bold">{cond}</span>. Result entry is blocked
                      until a Lab Supervisor grants an override.
                    </div>
                  </div>
                </div>
                <div className="space-y-2 pt-1">
                  ...
                  >
                    <ShieldAlert className="inline h-3.5 w-3.5 mr-1" /> Grant Override & Allow
                    Processing
                  </button>
                </div>
              </div>
            ...
            {currentCat?.parameters.map((p) => {
              ...
              return (
                <div
                  key={p.key}
                  className={cn(
                    "grid grid-cols-12 gap-3 items-center px-3 py-2 rounded-lg transition",
                    f.level === "critical" && "bg-red-50/50",
                    (f.level === "low" || f.level === "high") && "bg-amber-50/30",
                  )}
                ...
                >
                  ...
                  <div className="col-span-3">
                    <Input
                      ...
                      className="bg-white border-ink-200 font-mono"
                      placeholder="—"
                      disabled={
                        reagentStatus.blocked ||
                        (!!current.specimen?.condition &&
                          current.specimen.condition !== "Adequate" &&
                          !current.sampleConditionOverride)
                      }
                    />
                  </div>
          ...
          <div className="flex gap-2 mt-6">
            <Button
              ...
              disabled={reagentStatus.blocked}
            ...
            <Button
              ...
              disabled={!allFilled || reagentStatus.blocked}
          ...
          <div className="mt-8 pt-6 border-t border-ink-200">
            <h4 className="text-xs font-mono uppercase tracking-wider text-ink-400 mb-4">
              Sample Chain of Custody Audit Trail
            </h4>
            <div className="relative border-l-2 border-stone-200 pl-4 ml-2 space-y-4 text-xs">
              {current.chainOfCustody?.map((c, i) => (
                <div key={i} className="relative">
                  ...
                  <div className="font-semibold text-ink-800 capitalize">
                    {c.step.replace(/_/g, " ")}
                  </div>
                  <div className="text-ink-500">
                    {c.performedBy} · {c.location} · {formatDateTime(c.performedAt)}
                  </div>
                  {c.notes && (
      ...
      {queue.length === 0 ? (
        <div className="surface p-8">
          <EmptyState
            ...
            hint="No collected samples waiting for analysis."
          />
        </div>
      ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <ChevronRight className="h-4 w-4 inline text-stone-400" />
                    </td>
                  </tr>
                );
              })}
