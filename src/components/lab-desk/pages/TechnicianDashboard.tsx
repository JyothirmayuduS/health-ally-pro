...
const STEPS = [
  ...
  {
    to: "/lab/my-submissions",
    label: "Submissions",
    sub: "Supervisor sign-off",
    icon: Send,
    color: "sage",
  },
] as const;
  ...
  return (
    <div className="space-y-8" data-testid="technician-dashboard">
      ...
      {qcLocks.length > 0 ? (
        <div className="rounded-lg border border-red-200 bg-red-50/50 px-4 py-3 text-[13px] text-red-800 flex items-start gap-2.5">
          ...
          <div>
            ...
            due to QC failure. Please notify supervisor.
          </div>
        </div>
      ) : (
        <div className="rounded-lg border border-green-200 bg-green-50/50 px-4 py-3 text-[13px] text-green-800 flex items-start gap-2.5">
          ...
          <div>
            <strong>Quality Control Pass:</strong> All instruments calibrated and QC checks verified
            for today.
          </div>
        </div>
      )}
      ...
      <div className="grid gap-2 sm:grid-cols-4">
        {STEPS.map((step, i) => {
          ...
          return (
            ...
            >
              <div
                className={`flex h-9 w-9 items-center justify-center rounded-md ${STEP_STYLES[step.color]}`}
              >
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
        <KpiCard
          testid="tech-kpi-collect"
          label="Draw queue"
          value={stats.toCollect}
          hint="Collection"
          onClick={() => setKpiModalType("toCollect")}
        />
        <KpiCard
          testid="tech-kpi-specimens"
          label="My specimens"
          value={stats.specimens}
          hint="My samples"
          onClick={() => setKpiModalType("specimens")}
        />
        <KpiCard
          testid="tech-kpi-bench"
          label="At bench"
          value={stats.atBench}
          hint="Processing"
          onClick={() => setKpiModalType("atBench")}
        />
        <KpiCard
          testid="tech-kpi-critical"
          label="Critical alerts"
          value={stats.criticalToday}
          hint="Released today"
          accent="amber"
          onClick={() => setKpiModalType("criticalToday")}
        />
        <KpiCard
          testid="tech-kpi-signoff"
          label="Awaiting sign-off"
          value={stats.awaitingSignOff}
          hint="Submissions"
          onClick={() => setKpiModalType("awaitingSignOff")}
        />
        <KpiCard
          testid="tech-kpi-done"
          label="Submitted today"
          value={stats.submittedToday}
          hint="Results sent"
          onClick={() => setKpiModalType("submittedToday")}
        />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="surface border-l-4 border-plum p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link
              to="/lab/collection"
              className="text-[12px] font-medium text-plum hover:underline"
            >
              Draw queue →
            </Link>
          </div>
      ...
      <Dialog open={kpiModalType !== null} onOpenChange={(open) => !open && setKpiModalType(null)}>
        <DialogContent className="max-w-xl bg-white max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3 my-2 text-sm">
              ...
              <div className="space-y-2">
                {myOrders.filter((o) => o.status === "ordered").length === 0 ? (
                  <p className="text-ink-400 italic text-center py-4">
                    No backlog items awaiting sample collection.
                  </p>
                ) : (
                  myOrders
                    .filter((o) => o.status === "ordered")
            ...
            {kpiModalType === "specimens" && (
              <div className="space-y-2">
                {myOrders.filter(hasPhysicalSpecimen).length === 0 ? (
                  <p className="text-ink-400 italic text-center py-4">
                    No physical specimens currently logged in your batch.
                  </p>
                ) : (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-ink-400 mt-0.5">
                            Condition: {o.specimen?.condition || "Adequate"}
                          </div>
                        </div>
                        <StatusPill status={o.status} />
                      </div>
            ...
            {kpiModalType === "atBench" && (
              <div className="space-y-2">
                {myOrders.filter((o) => o.status === "collected" || o.status === "processing")
                  .length === 0 ? (
                  <p className="text-ink-400 italic text-center py-4">
                    No specimens currently at processing bench.
                  </p>
                ) : (
                  myOrders
                    .filter((o) => o.status === "collected" || o.status === "processing")
                  ...
                  criticalNotifications.map((n) => {
                    ...
                    return (
                      <div key={n.id} className="p-3 bg-stone-50 border rounded-lg text-ink-800">
                        ...
                        <div className="text-xs text-ink-500">
                          Notified at: {new Date(n.notifiedAt).toLocaleTimeString()}
                        </div>
            ...
            {kpiModalType === "awaitingSignOff" && (
              <div className="space-y-2">
                {myOrders.filter((o) => o.status === "validation").length === 0 ? (
                  <p className="text-ink-400 italic text-center py-4">
                    No submissions awaiting supervisor sign-off.
                  </p>
                ) : (
                  myOrders
            ...
            {kpiModalType === "submittedToday" && (
              <div className="space-y-2">
                {myOrders.filter(
                  ...
                  <p className="text-ink-400 italic text-center py-4">
                    No specimens submitted today yet.
                  </p>
                ) : (
                  myOrders
                    .filter(
