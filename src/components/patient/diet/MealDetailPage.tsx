...
export function MealDetailPage({ mealId }: { mealId: string }) {
  ...
  const confidence = meal.aiIntelligence ? Math.round(meal.aiIntelligence.confidence * 100) : null;
    ...
    <div className="relative w-full bg-[#F9F7F2]">
      ...
      <div className="mx-auto max-w-4xl px-5 pb-12 pt-6 lg:px-8 lg:pb-16 lg:pt-8">
        ...
        <div className="mt-2 flex flex-wrap gap-4 border-b border-[#EDEAE6] pb-6">
          ...
          <MetaChip
            icon={Zap}
            label={`P ${nutrition.proteinG}g · C ${nutrition.carbsG}g · F ${nutrition.fatG}g`}
            accent
          />
        </div>
        ...
        {meal.clinicalBenefits?.length ? (
          <section className="mt-8">
            <h3 className="mb-4 font-serif text-lg text-ink">Systemic Biological Optimization</h3>
        ...
        <section className="mt-8">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            ...
            {videosLoading ? (
              <span className="text-xs text-ink-muted">
                Loading {DIET_LANGUAGE_LABELS[videoLanguage]}…
              </span>
            ) : null}
          </div>
          ...
          {videos.length ? (
            ...
            <p className="text-sm text-ink-muted">
              No videos found for this language. Try another language.
            </p>
          )}
        </section>
