...
export default function Operations() {
  ...
  const stats = useMemo(() => {
    ...
    const turnaround = prescriptions
      ...
      .map(
        (r) => (new Date(r.collected_at!).getTime() - new Date(r.received_at).getTime()) / 60_000,
      );
    const avgTat = turnaround.length
    ...
    const statusMix = [
      {
        name: "Inbox",
        count: prescriptions.filter((r) => ["received", "in_review"].includes(r.status)).length,
      },
      {
        name: "Dispense",
        count: prescriptions.filter((r) => ["ready_to_dispense", "dispensing"].includes(r.status))
          .length,
      },
    ...
    const lowStock = drugs
      ...
      .map((d) => ({
        ...
        orderQty: Math.max(
          d.reorder_level * 2 - availableQty(batches.filter((b) => b.drug_id === d.id)),
          d.reorder_level,
        ),
      }));
  ...
  return (
    <div className="space-y-6" data-testid="pharmacy-operations">
          ...
          {stats.activeAlerts.slice(0, 3).map((a) => (
            ...
            >
              ...
              {a.action_to && a.action_label && (
                <Link to={a.action_to} className="btn-primary !h-8 !px-3 !text-[11px]">
                  {a.action_label}
                </Link>
              )}
            </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <KpiCard
          label="SLA breaches"
          value={stats.slaBreaches.length}
          hint="&gt;15 min in inbox"
          ...
          {stats.slaBreaches.length > 0 && (
            <div className="surface lg:col-span-2">
              <div className="flex items-center justify-between border-b border-ink-200 px-5 py-4">
                ...
                <Link to="/pharmacy/prescriptions" className="btn-primary !h-8 !text-[12px]">
                  Open inbox
                </Link>
              </div>
              ...
              {stats.expiring.length === 0 ? (
                ...
                stats.expiring.map(({ batch, drug, exp }) => (
                  <tr key={batch.id} className="border-b border-stone-100">
                    ...
                    <td
                      className={`px-4 py-3 ${exp.level === "critical" ? "font-medium text-clay" : "text-mustard"}`}
                    >
                      {exp.label}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      {batch.status === "active" ? (
                        <Button
                          ...
                          onClick={() => quarantineBatch(batch.id, "Expiry review")}
                        >
                          Quarantine
                        </Button>
                      ) : (
      ...
      {tab === "coldchain" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[12px] text-ink-400 mt-0.5">
                Schedule H/X and cold-chain integrity incident records — who acknowledged,
                corrective action, and affected stock status.
              </p>
            </div>
          ...
          {coldChainBreaches.length === 0 ? (
            <div className="surface py-12 text-center text-ink-400">
              No cold chain breaches logged.
            </div>
          ) : (
            <div className="surface overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {coldChainBreaches.map((b) => (
                    <tr
                      key={b.id}
                      className="border-b border-stone-100 hover:bg-stone-50/50 align-top"
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        {b.status === "resolved" ? (
                          <div>
                            <span className="rounded px-2 py-0.5 text-[10px] font-bold bg-sage-soft text-sage">
                              Resolved
                            </span>
                            {b.resolvedAt && (
                              <div className="text-[10px] text-ink-400 mt-0.5">
                                {new Date(b.resolvedAt).toLocaleString()}
                              </div>
                            )}
                          </div>
                        ) : (
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <span className="text-[12px] text-clay">
                      On hand {avail} · reorder {drug.reorder_level}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[13px] text-ink-600">
                    Order <strong>{orderQty}</strong> units
                  </span>
                  <Button
                    size="sm"
                    className="btn-primary"
                    onClick={() =>
                      window.alert(`PO draft created for ${drug.generic_name} × ${orderQty}`)
                    }
                  >
              ...
              {movements.map((m) => {
                ...
                return (
                  <tr key={m.id} className="border-b border-stone-100 hover:bg-stone-50/50">
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded-sm bg-ink-100 px-2 py-0.5 font-mono text-[10px] uppercase">
                        {m.type}
                      </span>
                    </td>
                    <td className="px-4 py-3 font-medium">{drug?.generic_name}</td>
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl border border-ink-200 w-full max-w-lg flex flex-col max-h-[90vh] overflow-hidden">
        <div className="px-5 py-4 border-b border-ink-200 bg-stone-50 flex items-center justify-between">
          <div>
            <h3 className="font-heading text-[15px] font-bold text-ink-900">
              Log Cold Chain Breach
            </h3>
            <p className="text-[11px] text-ink-500 mt-0.5">
              Regulatory record of temperature excursion and corrective action taken
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          <div>
            <label className="block text-[12px] font-bold text-ink-700 mb-1">
              Affected Batches (select all impacted)
            </label>
              ...
              {coldBatches.length === 0 ? (
                ...
                coldBatches.map((b) => {
                  ...
                  return (
                    <label
                      key={b.id}
                      className="flex items-center gap-2.5 px-3 py-2 cursor-pointer hover:bg-sky-50/40"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                      ...
                      <span className="font-mono text-[10px] text-ink-400">
                        {b.lot} · {b.qty} units · Exp {b.expiry}
                      </span>
                    </label>
                  );
                })
