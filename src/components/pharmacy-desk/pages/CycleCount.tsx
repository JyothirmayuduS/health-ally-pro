...
export default function CycleCount() {
  ...
  const racks = useMemo(
    () => [
      ...new Set(activeBatches.map((b) => findDrug(b.drug_id)?.location.rack).filter(Boolean)),
    ],
    [activeBatches],
  );
    ...
    <div className="space-y-6" data-testid="cycle-count">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {[
          ...
          {
            label: "With variances",
            value: stats.variances,
            icon: AlertTriangle,
            color: stats.variances > 0 ? "text-clay" : "text-ink-400",
          },
          {
            label: "Shortages",
            value: stats.shortages,
            icon: RefreshCw,
            color: stats.shortages > 0 ? "text-clay" : "text-ink-400",
          },
          ...
          <div key={s.label} className="surface p-3 flex items-center gap-3">
            ...
            <div>
              <div className={cn("font-mono text-[20px] font-bold leading-none", s.color)}>
                {s.value}
              </div>
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-[11px] text-ink-400">
          ...
          <span>
            {stats.counted} / {stats.total} batches
          </span>
        </div>
              ...
              return (
                ...
                >
                    ...
                    {!isSubmitted && variance !== null ? (
                      <span
                        className={cn(
                          ...
                          variance < 0 ? "text-clay" : variance > 0 ? "text-sage" : "text-ink-400",
                        )}
                      >
                        {variance > 0 ? "+" : ""}
                        {variance}
                  ...
                  <td className="px-4 py-3 text-right">
                    {isSubmitted ? (
                      <span className="rounded px-2 py-1 text-[10px] font-bold bg-sage-soft text-sage">
                        Submitted
                      </span>
                    ) : (
                      <Button
                        ...
                        className={cn(
                          ...
                          variance !== null &&
                            variance !== 0 &&
                            "border-clay/40 text-clay hover:bg-clay-soft/20",
                        )}
                        disabled={counted === ""}
      ...
      {stats.variances > 0 && (
        <div className="rounded-lg border border-clay/30 bg-clay-soft/20 px-4 py-3 text-[13px] text-ink-700">
          <strong className="text-clay">
            ⚠ {stats.variances} variance{stats.variances !== 1 ? "s" : ""} detected.
          </strong>{" "}
          Review shortages with your supervisor and log any discrepancies in the controlled
          register. Export the CSV for your count sheet records.
        </div>
      )}
      ...
      {stats.total > 0 && stats.counted === stats.total && (
        <div className="rounded-lg border border-sage/30 bg-sage-soft/30 px-4 py-3 text-[13px] text-ink-700">
          <strong className="text-sage">✓ Count complete.</strong> All {stats.total} batches have
          been physically counted and recorded this session.
        </div>
      )}
    </div>
  );
