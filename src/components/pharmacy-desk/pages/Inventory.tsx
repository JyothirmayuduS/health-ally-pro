...
import { usePharmacyStore, availableQty, isLowStock, fefoBatch } from "@/lib/pharmacy-desk/store";
...
export default function Inventory() {
  ...
  const filtered = useMemo(() => {
    return drugs.filter((d) => {
      ...
      return [d.generic_name, ...d.brand_names, d.sku, d.location.location_code]
        .join(" ")
        .toLowerCase()
        .includes(q);
    });
  ...
  return (
    <div className="space-y-6">
      <SectionLabel
        action={
          <div className="flex gap-2">
            ...
            <Link to="/pharmacy/map" className="btn-outline !h-8 !px-3 !text-[12px]">
              <Map className="mr-1.5 h-3.5 w-3.5" /> Storage map
            </Link>
            <Button size="sm" className="btn-primary" onClick={() => setReceiveOpen(true)}>
      ...
      <div className="surface flex flex-wrap gap-3 p-4">
        <Input
          placeholder="Search drug, SKU, location…"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          className="min-w-[240px] flex-1 border-ink-200 bg-white"
        />
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7}>
                  <EmptyState icon={Briefcase} title="No items" />
                </td>
              </tr>
            )}
            {filtered.map((d) => {
                ...
                <tr key={d.id} className="border-b border-stone-100 hover:bg-stone-50/50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="font-mono text-[11px] text-ink-500">
                      {fmtInr(d.purchase_cost ?? 0)} cost
                    </div>
                    <div className="font-mono text-[12px] font-semibold text-sage">
                      {fmtInr(d.unit_price)} sell
                    </div>
                    <div className="text-[10px] text-ink-400">
                      {fmtMargin(d)} · {unitLabel(d.form)}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 font-mono">
                    {avail} <span className="text-ink-400">/ reorder {d.reorder_level}</span>
                  </td>
                  <td className="px-4 py-3 text-[12px]">
                    {fefo ? (
                      <>
                        {fefo.lot} ·{" "}
                        <span className={exp?.level === "critical" ? "text-clay" : ""}>
                          {exp?.label}
                        </span>
                        {fefo.purchase_cost_per_unit != null && (
                          <div className="font-mono text-[10px] text-ink-400">
                            @ {fmtInr(fefo.purchase_cost_per_unit)}
                          </div>
                        )}
                      </>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    {low ? (
                      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-clay">
      ...
      <div className="surface">
        <div className="border-b border-ink-200 px-5 py-3 font-mono text-[10px] uppercase tracking-wider text-ink-400">
          Recent movements
        </div>
        ...
        <DialogContent className="max-w-md border-ink-200">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <Label>
                  Purchase cost {selectedDrug ? unitLabel(selectedDrug.form) : "per unit"}
                </Label>
                <Input
                  type="number"
                  step="0.01"
            ...
            {selectedDrug && (
              <p className="rounded-md bg-stone-50 px-3 py-2 text-[12px] text-ink-600">
                Formulary sell price:{" "}
                <strong className="text-sage">{fmtInr(selectedDrug.unit_price)}</strong>{" "}
                {unitLabel(selectedDrug.form)}
