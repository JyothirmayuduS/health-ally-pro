...
import { CalendarDays, PlusCircle, Clock, CheckCircle2, XCircle, FileText, X } from "lucide-react";
  ...
  return (
    <div className="space-y-6" data-testid="staff-leave-portal">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 surface px-5 py-4 shadow-soft border border-ink-100 rounded-lg">
        <div>
          <h2 className="font-heading font-semibold text-ink-950 text-[15px]">
            Leave Management Portal
          </h2>
          <p className="text-[12px] text-ink-400 mt-0.5">
            Logged in as {profile.staffName} ({profile.role}) · {profile.department}
          </p>
        </div>
        <button
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((req) => {
              ...
              return (
                <div
                  key={req.id}
                  className="px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-bone/10 transition"
                >
                  <div className="flex items-start gap-4">
                    {/* Leave type badge */}
                    <div
                      className={`mt-0.5 px-2.5 py-1 rounded-md border text-[10.5px] font-bold uppercase shrink-0 ${typeInfo?.color}`}
                    >
                      {typeInfo?.label}
                    </div>
                    <div className="space-y-0.5">
                      <p className="font-semibold text-ink-900 text-[13.5px]">
                        ...
                        <span className="ml-2 text-[11px] text-ink-400 font-normal">
                  ...
                  <span
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-[11px] font-semibold shrink-0 ${statusInfo.cls}`}
                  >
            ...
            <div className="px-6 py-5 border-b border-stone-100 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11.5px] text-ink-400 mt-0.5">
                  {profile.staffName} · {profile.department}
                </p>
              </div>
              <button
                type="button"
