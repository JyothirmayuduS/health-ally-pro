...
function compareDelta(current: string, previous?: string | number): "up" | "down" | "same" | null {
  ...
  const b =
    typeof previous === "number"
      ? previous
      : Number.parseFloat(String(previous).replace(/[^\d.]/g, ""));
...
function formatReadingSummary(row: VitalsReading): string {
  const parts = [
    ...
    row.bodyMarkers?.length
      ? `${row.bodyMarkers.length} area${row.bodyMarkers.length === 1 ? "" : "s"} marked`
      : null,
  ].filter(Boolean);
  return parts.join(" · ") || "—";
}
...
export function VitalsCaptureWorkspace({
  ...
  const abnormalCount = CORE_VITAL_FIELDS.filter(
    (f) => fieldFlag(f.key, form[f.key]) !== "normal",
  ).length;
          ...
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {abnormalCount > 0 ? (
                <span
                  className={cn(
                    "rounded-full px-2.5 py-1 text-[11px] font-semibold lg:px-3 lg:py-1.5 lg:text-xs",
                    VITAL_FLAG_BADGE.critical,
                  )}
                >
            ...
            {CORE_VITAL_FIELDS.map((field) => {
              ...
              return (
                <label
                  key={field.key}
                  className={cn(
                    "block rounded-2xl border p-2.5 transition-colors lg:p-3",
                    VITAL_FLAG_STYLE[flag],
                  )}
                >
                  <span className="flex items-center justify-between gap-1 text-[9px] font-semibold uppercase tracking-wide text-[#8A8F8C] lg:text-[10px]">
                    ...
                    {flag !== "normal" ? (
                      <span
                        className={cn(
                          "rounded-full px-1.5 py-0.5 text-[8px] normal-case lg:text-[9px]",
                          VITAL_FLAG_BADGE[flag],
                        )}
                      >
                  ...
                  <div className="mt-1.5 flex items-end gap-1 lg:mt-2 lg:gap-1.5">
                    ...
                    />
                    <span className="shrink-0 pb-1 text-[10px] text-[#8A8F8C] lg:text-xs">
                      {field.unit}
                    </span>
                  </div>
                  {delta && lastReading ? (
        ...
        <aside className="space-y-3 lg:space-y-4">
          ...
          {lastReading ? (
            <section className="rounded-[20px] border border-[#EDEAE6] bg-[#F7FAF6] p-3 lg:p-4">
              ...
              <p className="mt-0.5 text-xs text-[#8A8F8C]">
                {formatVitalsRecordedAt(lastReading.recordedAt)}
              </p>
          ...
          <section className="rounded-[20px] border border-[#EDEAE6] bg-white p-3 lg:p-4">
            ...
            <ul className="mt-3 max-h-[280px] space-y-2 overflow-y-auto lg:max-h-[320px]">
              {history.length === 0 ? (
                <li className="rounded-xl bg-[#FAFAF8] p-3 text-sm text-[#8A8F8C]">
                  No vitals yet.
                </li>
              ) : (
                history.map((row) => (
                  <li key={row.id} className="rounded-xl border border-[#F0EDE8] px-3 py-2.5">
                    <p className="text-xs font-medium text-[#8A8F8C]">
                      {formatVitalsRecordedAt(row.recordedAt)}
                    </p>
                    <p className="mt-1 text-sm font-medium text-[#1B3B2E]">
                      {formatReadingSummary(row)}
                    </p>
                  </li>
                ))
              )}
